...
class CoroutineCallAdapterFactory private constructor() : CallAdapter.Factory() {
    ...
    override fun get(
        ...
        return if (rawDeferredType == MResponse::class.java) {
    ...
    private class ResponseCallAdapter<T>(
        private val responseType: Type
    ) : CallAdapter<T, Deferred<retrofit2.Response<T>>> {

        override fun responseType() = responseType

        override fun adapt(call: Call<T>): Deferred<retrofit2.Response<T>> {
            val deferred = CompletableDeferred<retrofit2.Response<T>>()
